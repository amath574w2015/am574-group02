...
\begin{itemize}
 \item {Multiply by test function:}    \[ 
...
\vspace{-3mm}	 	
\item {Galerkin ansatz:}  	 \[ q(t,\xi) \sim \sum_{k=0}^N Q^{(k)}(t) \, \varphi^{(k)}(\xi) \]	 	
...
\begin{frame}{Buffer Regions}
 \only <1>{ \begin{center}
  \includegraphics[height=30 mm]{buffer1}
 \end{center} }
  \only <2>{ \begin{center}
  \includegraphics[height=30 mm]{buffer2}
 \end{center} }
\end{frame}
